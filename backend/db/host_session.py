...
async def set_wallet_id(_id: str, wallet_id: str):
    ...
    await collection.update_one(
        {"_id": ObjectId(_id)}, {"$set": {"wallet_id": wallet_id}}
    )
...
async def set_current_processing(host_session_id, session_id):
    ...
    """
    await collection.update_one(
        {"_id": ObjectId(host_session_id)},
        {
            "$set": {
                "current_processing": {"$ref": "requests", "$id": ObjectId(session_id)}
            }
        },
    )
...
async def set_current_processing_as_none(host_session_id: str):
    ...
    """
    await collection.update_one(
        {"_id": ObjectId(host_session_id)}, {"$set": {"current_processing": None}}
    )
    ...
    for host_id, timestamp in data:
        ...
        to_update.append(
            UpdateOne(
                ...
                {
                    "$set": {"status": status, "last_ping": timestamp},
