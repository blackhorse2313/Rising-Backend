...
producer = KafkaProducer(
    bootstrap_servers=[config.KAFKA_SERVER_URL],
    api_version=(0, 10),
...
async def create_img2img_request(data):
    ...
    parameters = util.get_qrimg_prompt(data.prompt, data.image)
    db_data = {
        "status": "available",
    ...
    response = await collection.insert_one(db_data)
    await create_producer(str(response.inserted_id))
    image_data = await get_request_status(response.inserted_id)

    return {"session_id": str(response.inserted_id), "image_data": image_data}
...
async def create_avatar_request(data: schema.CreateRequestAvatar):
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail="session_id is invalid make sure its 24 character hex string.",
        )
...
async def create_txt2img_request(data):
    ...
    return {"session_id": str(response.inserted_id), "image_data": dict(image_data)}
...
prompts = {
    "library": (
        "staring up into the infinite maelstrom library, endless books, flying books, spiral staircases, nebula, ominous, cinematic atmosphere, negative dark mode, mc escher, art by sensei jaye, matrix atmosphere, digital code swirling, matte painting, laboratory , sharp , raytracing , light tracing , 8k , ultra preset",
    ...
    "mech-robot": (
        "mecha, robot  ven, war, gundam,1robot, high quality, high resolution. detailed body, detailed, legs , detailed face , detailed metal,8k, ultra realistic, icy background ,explosion in the background, fire  long sword  in right hand, shield in left hand, darker blacks,finely detailed,best quality, extra sharp , 16x anistrophic filtering",
...
async def get_request_status(id: str):
    ...
    if status == "complete":
        payload = {
            ...
            "output": {
                "images": [{"base64": i} for i in response.get("output").get("images")]
            },
        }
        return payload
        ...
        return await get_request_status(id)
...
async def _get_request_upscale(request, session_id):
    image = await aws_client.download_file(
        f"{session_id}_UPSCALE{request.get('parameters').get('upscaling_resize')}X.png"
    )
    if image is None:
        raise HTTPException(status_code=404, detail="Image not found")
    return {"status": "complete", "image": image}
...
async def request_completed_single_output(session_id: str, image_data):
    """
    Updates the status of the request to available.
    {"status":"complete","processed_at":datetime.utcnow()}
    """
    status = await collection.update_one(
...
async def request_completed_txt2img(session_id: str, image_data):
    """
    Updates the status of the request to available.
    {"status":"complete","processed_at":datetime.utcnow()}
    """
...
async def set_available_request(new_id, host_session_id) -> Union[None, Dict]:
    """
    ...
    :return:
    """
    request = await collection.find_one_and_update(
        {"_id": ObjectId(new_id)},
        {
            "$set": {
                "status": "processing",
...
async def __create_payload(request):
    if request.get("request_type") == "txt2img":
        ...
        request.get("request_type") == "qr1" or request.get("request_type") == "img2img"
    ):
        url = request.get("url")
        parameters = request.get("parameters")
        if len(url) > 22:
