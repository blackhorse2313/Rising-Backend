...
# from immudb api.tasks import create_payment
app = FastAPI(
...
async def assign_request(newId=Header(), host_session_id=Header(), version=Header()):
    ...
    payload = await requests_col.set_available_request(newId, host_session_id)
...
async def submit_request(
    data: Union[schema.SubmitImageRequesttxt2img, schema.SubmitImageRequestUpscale],
    host_session_id=Header(),
    version=Header(),
):
    if data.request_type == "txt2img":
        ...
        for i in enumerate(data.images):
            temp.append(i[1])
        await requests_col.request_completed_txt2img(data.session_id, temp)
    elif data.request_type == "upscale":
        response = await requests_col.collection.find_one(
        ...
        image_data = base64.b64decode(data.image.split(",", 1)[0])
        await requests_col.request_completed_single_output(data.session_id, image_data)
    elif data.request_type == "qr1":
        await requests_col.request_completed_single_output(data.session_id, data.image)
    elif data.request_type == "img2img":
        await requests_col.request_completed_single_output(data.session_id, data.image)
    print("Creating Payment")
    asyncio.get_event_loop().create_task(
        host_session.set_current_processing_as_none(host_session_id)
    )
    return {"success": True}
...
async def create_txt2img_request(data: schema.CreateRequestCustom):
    response = await requests_col.create_txt2img_request(data)
    if data.discord_id:
        await users_col.append_request(data.discord_id, response.get("session_id"))
    return response


@app.post("/v3/user/create-img2img", response_model=schema.CreateRequestResponse)
async def create_img2img_request(data: schema.CreateRequestImg2Img):
    response = await requests_col.create_img2img_request(data)
    await users_col.append_request(data.discord_id, response.get("session_id"))
    return response
...
async def create_qr1_request(data: schema.CreateQR1Request):
    response = await requests_col.create_qr1_request(data)
    if data.discord_id:
        await users_col.append_request(data.discord_id, response.get("session_id"))
    return response
...
async def set_wallet_id(
    data: schema.SetWalletIDRequest, host_session_id=Header(), version=Header()
):
